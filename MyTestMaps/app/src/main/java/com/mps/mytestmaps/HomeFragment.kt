...
class HomeFragment : Fragment(), OnMapReadyCallback, PermissionsListener {
    ...
    companion object { private val DROPPED_MARKER_LAYER_ID = "DROPPED_MARKER_LAYER_ID" }
    ...
    override fun onExplanationNeeded(permissionsToExplain: MutableList<String>?) = toast("user_location_permission_explanation")
    ...
    override fun onPermissionResult(granted: Boolean) {
        if (granted && mapboxMap != null) {
            ...
            toast("user_location_permission_not_granted")
        }
    }
                    ...
                    if (response.body() != null) {
                        ...
                        if (results.size > 0) {
                            ...
                            mapboxMap!!.getStyle { style ->
                                if (style.getLayer(DROPPED_MARKER_LAYER_ID) != null) {
                                    toast("location place name ${feature.placeName()}")
                                }
                            }
                        } else {
                            toast("lokasi tidak ada hasil")
                        }
                    }
                }
